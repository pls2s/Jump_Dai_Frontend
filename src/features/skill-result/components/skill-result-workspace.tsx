...
export function SkillResultWorkspace({ courseId, courseTitle }: { courseId: string; courseTitle: string }) {
  ...
  return (
    <ContentContainer className="max-w-6xl">
      ...
      <Card className="mt-7 flex flex-col items-start justify-between gap-5 border-blue-200 bg-blue-50 p-5 sm:flex-row sm:items-center sm:p-6"><div className="flex items-start gap-3"><ClipboardCheck className="mt-0.5 size-5 shrink-0 text-blue-800" aria-hidden="true" /><div><h2 className="font-semibold text-blue-950">Skill evidence is the next step</h2><p className="type-body-small mt-1 text-blue-900">Function 16 will organize the assessment and practical evidence into a learner portfolio.</p></div></div><ButtonLink href={`/learner/courses/${courseId}/skill-evidence`}>View skill evidence<ArrowRight className="size-4" aria-hidden="true" /></ButtonLink></Card>

...
import { Button, ButtonLink, Card, Spinner, useToast } from "@/components/ui";
import {
  appendPreviewState,
...
function DemoCredentialWorkspace({ courseId, credentialId, previewState }: { courseId: string; credentialId: string; previewState: PortfolioPreviewState }) {
  ...
  async function copyCredentialLink() {
    try {
      ...
      showToast({ tone: "success", title: "Credential link copied" });
    } catch {
      showToast({ tone: "error", title: "Credential link wasn’t copied", description: "Use the current browser address instead." });
    }
  }
  ...
  async function claimCredential() {
    ...
    if (!result.ok) {
      showToast({ tone: "error", title: "Credential wasn’t issued", description: result.message });
      setIsIssuing(false);
      return;
    }
    setPortfolio(result.portfolio);
    showToast({ tone: "success", title: "Credential issued", description: "Your frontend demo credential was saved to this browser." });
    setIsIssuing(false);
  }

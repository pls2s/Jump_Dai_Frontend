...
`Skill Result → Portfolio Overview → Skill Detail → Evidence Result → Credential Requirements → Credential Preview`
...
No new localStorage subsystem was added. Portfolio evidence, verification, eligibility, issue date, and the deterministic `SS-DEMO-*` credential identifier are derived from the persisted learner journey, learner profile, and course configuration. This prevents duplicated status records from becoming inconsistent after refresh.
